...
function draw() {
  ...
  cloud.y = Math.round(random(width/4,height/5));
  cloud.velocityX = -3;
  trex.collide (cloud); 
  cloud.depth = -2;
  cl_grp.add(cloud);
...
//  console.log("score=" , i);
  ...
  text (zero(i)+Math.round(i),width/2+155,50);
  text ("HIGH SCORE =",width/2 - 60,50);
  text (zero(highscore)+Math.round(highscore), width/2 + 50,50);
  //console.time("t");
...
else if (xyz > 9 && xyz <= 99){
  return "000";
} 
else if (xyz > 99 && xyz <= 999){
  return "00";
}
else if (xyz > 999 && xyz <= 9999){
  return "0";
} 
} 